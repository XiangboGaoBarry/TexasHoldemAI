...
class NolimitholdemEnv(Env):
    ...
    def _extract_state(self, state):
        ...
        obs[53] = float(all_chips[0])
        obs[54] = float(all_chips[0])
        obs[55] = self.game.round_counter
        extracted_state['obs'] = obs

        if state['hist_bets']:

            if not player_id:
                hist_bet_flatten = (bet[0] for bet in state['hist_bets'][20:] + state['hist_bets'][0:20])
            else:
                hist_bet_flatten = (bet[0] for bet in state['hist_bets'][:])
            for i, bet in enumerate(hist_bet_flatten):
                obs[56 + i] = bet
    ...
    def get_payoffs(self):
        ...
        '''
        win_condition, winner, pay_off = self.game.get_payoffs()
        return win_condition, winner, np.array(pay_off)
