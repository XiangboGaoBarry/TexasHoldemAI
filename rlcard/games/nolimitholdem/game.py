...
class NolimitholdemGame(Game):
    ...
    def __init__(self, allow_step_back=False, num_players=2):
        ...
        self.init_chips = [1000] * num_players
    ...
    def step(self, action):
        ...
        bet_amount, next_game_pointer = self.round.proceed_round(self.players, action)


        for i in range(5):
            if i == 4:
                self.players_hist_bets[self.game_pointer * 20 + self.round_counter * 5 + i] = \
                    (self.players_hist_bets[self.game_pointer * 20 + self.round_counter * 5 + i][0] + bet_amount, 1)
                break
            if self.players_hist_bets[self.game_pointer * 20 + self.round_counter * 5 + i][1] == 0:
                self.players_hist_bets[self.game_pointer * 20 + self.round_counter * 5 + i] = (bet_amount, 1)
                break

        self.game_pointer = next_game_pointer
