...
class LimitholdemGame(object):
    ...
    def game_over_print(self, win_condition, winners, payoffs):
        ...
        print("The winner is Player {}. It wins {} chips.".format(np.where(np.array(winners) == 1)[0], max(payoffs)))
    ...
    def get_payoffs(self):
        ...
        win_condition, winners, chips_payoffs = self.judger.judge_game(self.players, hands)
        payoffs = np.array(chips_payoffs) / (self.big_blind)
        return win_condition, winners, payoffs
